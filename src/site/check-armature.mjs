...
try{
 await page.goto(`${process.env.SITE_URL||'http://127.0.0.1:5175/'}#machine/armature`);
 await page.getByRole('heading',{name:'Armature',exact:true}).waitFor();
 const isolated=page.getByRole('checkbox',{name:'Isolate selected part',exact:true}),detail=page.locator('.daily-part-detail');
 assert.equal(await isolated.isChecked(),false);assert.match(await detail.textContent(),/Moving iron armature/);
 ...
 assert.equal(await isolated.isChecked(),false);assert.match(await detail.textContent(),/Moving iron armature/);
 await page.getByRole('button',{name:'The hinged iron armature and its attached hammer.',exact:true}).click();
 assert.match(await page.locator('.daily-answer').textContent(),/That’s right/);
 await page.screenshot({path:'/tmp/howthingswork-armature-lesson.png',fullPage:true});
 await page.setViewportSize({width:390,height:844});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
 assert.deepEqual(errors,[]);
 console.log('PASS armature browser: dedicated connected route, three stepped experiments and whole-bell outcomes, reset context, quiz and mobile.');
