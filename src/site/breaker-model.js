...
export function createBreakerModel(){
 ...
 supply.userData.explosionRigid=true;
 ...
 plunger.userData.explosionRigid=true;
 ...
 const wire=points=>{const conductor=new THREE.Group();wiring.add(conductor);points.slice(1).forEach((p,i)=>rod(points[i],p,.025,'clay',conductor));};
 ...
 for(const assembly of [frame,magnet,contact,fixed,arc,load,wiring])assembly.userData.explosionPieces=true;
 ...
 let pull=0,angle=0,tripped=false,elapsed=0,lastClock=0,awaitingReset=false;
 ...
 return {state:{current,pull,angle,tripped,elapsed,overload,awaitingReset,complete:tripped&&angle>=.85&&pull<=1e-8},readings:[r('Your result',awaitingReset?'Reset the breaker to close the contacts':angle>0?'Contacts separated · lamp off':tripped?'Latch released · contact opening':overload?'Overload · run to watch the trip':current?'Contacts closed · lamp lit':'Supply off'),r('Mechanism',angle>=.85?'Open and latched off':tripped?'Opening spring lifts the arm':pull>0?'Core withdraws the retaining hook':'Hook holds the arm closed','Using Reset the breaker closes the contact. Leaving an overload in place makes the restored current trip it again.'),r('Circuit current',current.toFixed(3)+' A','The lamp and sensing coil carry the same series current. Any contact gap interrupts both.'),r('Core withdrawal',(pull/.22*100).toFixed(0)+'% of release travel','The hook clears at 100%. After current stops, the core returns even though the contacts stay open.'),r('Contact opening',(angle*180/Math.PI).toFixed(1)+'°','The opening spring turns the contact arm after the hook releases it.'),r('Time observed',elapsed.toFixed(2)+' s','Motion is deliberately slowed. This elapsed time is not a product trip curve.'),r('Trip model','Magnetic release above 1.5 A','Illustrative threshold and slowed motion, not a rated breaker or trip curve.')]};},{animated:true});
 function advance(dt){if(awaitingReset||result.getState().complete||!Number.isFinite(dt)||dt<=0)return result.update();let remaining=dt;
 ...
 result.reset=initialState=>{awaitingReset=initialState?.tripped===true;pull=0;angle=awaitingReset ? .85 : 0;tripped=awaitingReset;elapsed=0;lastClock=0;};
 result.actions=[{label:'Reset the breaker',run(){pull=0;angle=0;tripped=false;awaitingReset=false;result.update();}}];
 result.playback={label:'Run the circuit',stepLabel:'Advance the trip mechanism',description:'Follow a slowed magnetic trip. In reset experiments, first use Reset the breaker to close the prepared open contact.',blockedReason:'Use Reset the breaker to close the contact before running.',advance,step:()=>advance(.2),complete:()=>!awaitingReset&&result.getState().complete,blocked:()=>awaitingReset};
