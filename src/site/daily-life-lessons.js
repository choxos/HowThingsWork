...
import {binocularsLesson} from './binoculars-lesson.js';
import {telescopesLesson} from './telescopes-lesson.js';
import {lensesLesson} from './lenses-lesson.js';
import {mirrorsLesson} from './mirrors-lesson.js';
import {airbagWarningIndicatorLesson} from './airbag-warning-indicator-lesson.js';
import {crashSensorLesson} from './crash-sensor-lesson.js';
import {autopilotLesson} from './autopilot-lesson.js';
import {inertialAccelerometerLesson} from './inertial-accelerometer-lesson.js';
import {inertialGuidanceLesson} from './inertial-guidance-lesson.js';
import {sparkPlugElectrodesLesson} from './spark-plug-electrodes-lesson.js';
import {seismographRecordingLesson} from './seismograph-recording-lesson.js';
import {verticalSeismographSpringLesson} from './vertical-seismograph-spring-lesson.js';
import {earthquakeLocationLesson} from './earthquake-location-lesson.js';
import {seismicWavesLesson} from './seismic-waves-lesson.js';
import {verticalSeismographPendulumLesson} from './vertical-seismograph-pendulum-lesson.js';
import {horizontalSeismographPendulumLesson} from './horizontal-seismograph-pendulum-lesson.js';
import {seismographLesson} from './seismograph-lesson.js';
import {feedbackMechanismLesson} from './feedback-mechanism-lesson.js';
import {sensorsAndDetectorsLesson} from './sensors-and-detectors-lesson.js';
import {electronicIgnitionLesson} from './electronic-ignition-lesson.js';
...
import {threeAxisLesson, cadDesignLesson, laserScanningLesson} from './printing-lessons.js';
...
export const dailyLifeLessons = {
  'Raft':raftLesson,
  'Liquid crystal display':lcdLesson,
  'Polarized light':polarizedLightLesson,
  'Microscopes':microscopesLesson,
  'Binoculars':binocularsLesson,
 'Telescopes':telescopesLesson,
 'Lenses':lensesLesson,
  'Mirrors':mirrorsLesson,
  'Airbag warning indicator':airbagWarningIndicatorLesson,
  'Crash sensor':crashSensorLesson,
  'Autopilot':autopilotLesson,
  'Inertial accelerometer armature, spring, and coils':inertialAccelerometerLesson,
  'Inertial guidance':inertialGuidanceLesson,
  'Seismograph recording pen and moving paper':seismographRecordingLesson,
  'Vertical-seismograph suspension spring':verticalSeismographSpringLesson,
  'Earthquake location by arrival times':earthquakeLocationLesson,
  'Seismic waves':seismicWavesLesson,
  'Vertical seismograph pendulum':verticalSeismographPendulumLesson,
  'Horizontal seismograph pendulum':horizontalSeismographPendulumLesson,
  'Seismograph':seismographLesson,
  'Feedback mechanism':feedbackMechanismLesson,
  'Sensors and detectors':sensorsAndDetectorsLesson,
  'Electronic ignition':electronicIgnitionLesson,
  'Spark gap':sparkGapLesson,
  'Solenoid':solenoidLesson,
  'Spark-plug electrodes and ceramic insulator':sparkPlugElectrodesLesson,
  ...
  'Cylinder lock': {
    ...
    steps: [
      ...
      {title:'Close and lock again',body:'Keep the latch retracted while closing the door. Return the key to zero as the latch spring extends the bolt and turns the cam home. Here Play prescribes that return instead of calculating spring torque. Withdraw the upright key and watch the pin springs restore the blocking arrangement.'},
    ],
    parts: [
    ...
    tryIt: [
      {title:'Open the door',instruction:'Start with the matching key withdrawn. Press Play, or advance in small steps.',observe:'The key aligns the pins, the cam retracts the latch, and the door swings open.',reset:true,cutaway:true,isolate:false,view:'reset',values:{operation:0,insertion:0,keyPattern:0,turn:0,door:0}},
      {title:'One cut is enough',instruction:'Start with one incorrect cut. Run the opening action and inspect the third pin joint.',observe:'The third joint misses the shear line. The plug, latch and door stay still. Withdraw the key before choosing the matching pattern.',reset:true,cutaway:true,isolate:false,part:'pins',view:'side',values:{operation:0,insertion:0,keyPattern:1,turn:0,door:0}},
      {title:'Right key, wrong position',instruction:'This setup attempts a 70-degree turn with the matching key halfway in. Finish inserting it, then try turning again.',observe:'The blocked turn is discarded, so Actual plug turn stays at zero even when the key becomes seated. A fresh turn now moves the cam.',reset:true,cutaway:true,isolate:false,part:'pins',view:'side',values:{operation:0,insertion:0.5,keyPattern:0,turn:70,door:0}},
      {title:'Close and relock',instruction:'Start with the door open and latch retracted. Run the selected closing action, or advance in steps.',observe:'The door closes first. The prescribed spring return moves the cam home, then withdrawing the key lets the pin springs restore the obstruction.',reset:true,cutaway:true,isolate:false,view:'reset',values:{operation:1,insertion:1,keyPattern:0,turn:80,door:65}},
      {title:'Too high and too low both block',instruction:'Run the opening action with the Different key pattern. Compare the five pin joints with the blue boundary.',observe:'Some joints sit too high and others too low. All five miss the boundary, so neither the plug nor the latch can move.',reset:true,cutaway:true,isolate:false,part:'pins',view:'side',values:{operation:0,insertion:0,keyPattern:2,turn:0,door:0}},
      {title:'How far must the latch retract?',instruction:'At 42 degrees, try opening the door. Then increase Attempt to turn to 43 degrees and try again.',observe:'At 42 degrees the tip has passed the near edge, but has not reached the model’s extra clearance margin. At 43 degrees the door can move. Strike clearance uses the closed-door reference even after opening.',reset:true,cutaway:true,isolate:false,part:'latch-drive',view:'back',values:{operation:0,insertion:1,keyPattern:0,turn:42,door:0}},
    ],
    deeper: [
      ...
      {title:'Why the key can slide out',body:'At zero plug angle, the lower pins again line up with their upper drivers. As the blade slides, the slopes on either side of its cut valleys let the spring-loaded pins rise and fall. When the plug is turned, the lower pins rotate away from their driver bores and cannot rise normally; the key stays captured. Return it upright before withdrawing.'},
    ],
    ...
    limits: 'An enlarged five-pin cylinder drives an illustrative roller-and-slot spring latch. The door panel and half-cylinder walls are removed in the cutaway. Other locks use different cam and latch linkages. The flat cuts allow a small alignment range, but this model additionally requires full seating before turning. Actual pin alignment and full seating are separate checks. An 80° stop avoids the roller linkage’s dead-center position. Motion is prescribed by geometry: friction, spring torque, pin-tip curvature, wear and latch bevels are not simulated. Close this square latch with the key turned to retract it.',
    sources: [{title:'Schlage: cylinder components and terminology',url:'https://www.schlage.com/content/dam/sch-us/documents/pdf/installation-manuals/P513-325.pdf'},{title:'Linus Yale: divided pins and key withdrawal alignment',url:'https://patents.google.com/patent/US48475A/en'}],
  ...
  'Lever lock': {
    ...
    tryIt: [
      {title:'A wrong shoulder stops the drive',instruction:'This key has one shoulder too high. Run the selected opening action.',observe:'All three levers rise, but the middle gate misses the path. The turn stops at the drive stage and the door remains shut.',reset:true,cutaway:true,isolate:false,part:'lock',values:{operation:0,keyPattern:1,insertion:0,turn:0,door:0}},
      {title:'Aligned, but not moving yet',instruction:'The matching key is inserted and turned to 135 degrees. Inspect the key drive and then advance it.',observe:'The gates are already clear. The roller must reach the side of its slot before the bolt moves.',reset:true,cutaway:true,isolate:false,part:'lock',values:{operation:0,keyPattern:0,insertion:1,turn:135,door:0}},
      {title:'Let the springs reset the plates',instruction:'The bolt is fully retracted at 270 degrees. Advance to the end of the turn.',observe:'The plates lower into their end pockets while the deadbolt stays retracted. The key can then come out.',reset:true,cutaway:true,isolate:false,part:'lock',values:{operation:0,keyPattern:0,insertion:1,turn:270,door:0}},
      {title:'Open the doorway',instruction:'Start from the matching key and run the selected action.',observe:'The key drives the whole sequence and comes out. The door opens only once the bolt clears the strike.',reset:true,cutaway:true,isolate:false,part:'lock',values:{operation:0,keyPattern:0,insertion:0,turn:0,door:0}},
      {title:'Too little lift also blocks the bolt',instruction:'All three shoulders are too low. Run the opening action and compare Lever angles with the matching-key experiment.',observe:'The plates rise less than needed. The stump cannot enter every gate, so the drive stops with the bolt extended and the door shut.',reset:true,cutaway:true,isolate:false,part:'lock',values:{operation:0,keyPattern:2,insertion:0,turn:0,door:0}},
      {title:'Close the open door and relock',instruction:'Start with the door open, the bolt retracted and the matching key seated after its full opening turn. Run Close and relock.',observe:'The door closes first. The key reverses, the stump crosses the aligned gates, and the bolt enters the strike. The plates lower and the key comes out.',reset:true,cutaway:true,isolate:false,part:'system',values:{operation:1,keyPattern:0,insertion:1,turn:360,door:65}},
    ],
    deeper: [
    ...
    sources: [{title:'Lever dead-locks: fixed pivots, key shoulders, gates and bolt drive',url:'https://patents.google.com/patent/GB2228286A/en'},{title:'Lever locks and keys: end pockets, stump and reverse operation',url:'https://patents.google.com/patent/GB2293409A/en'}],
  ...
  'Keys': {
    ...
    steps: [
      {title:'Choose a cut while the key is out',body:'Select one of five positions, counted from the blade tip toward the bow. Raise or lower that cut. The other four retain their matching heights. Moving the selection transfers the change to the new cut and restores the previous one.'},
      {title:'Locate the cuts',body:'Seating the key places each cut beneath its intended pin. The spaces between cuts matter as well as their heights. Compare Applied cut change with the requested value; a blade limit can clip the request.'},
      ...
      {title:'Test one change',body:'Seat the edited key. A high cut leaves its lower pin across the boundary; a low cut leaves the upper driver across it. Compare Selected pin joint with the blue guide before trying to turn. At full seating its signed offset equals the applied cut change.'},
    ...
    parts: [
      ...
      {name:'Shear line',role:'The boundary that all the pin joints must clear to permit rotation; the blue line is a drawn reference, not a physical part.'},
      {name:'Keyway floor and pin springs',role:'Support the blade and keep the pins following its sloping surface during insertion and withdrawal.'},
    ],
    tryIt: [
      {title:'Open with a matching shape',instruction:'Start with the matching key out and the cut change at zero. Run Unlock and open.',observe:'The key inserts, all five joints align, the plug turns and the door opens.',reset:true,cutaway:true,isolate:false,part:'plug',view:'side',values:{operation:0,insertion:0,selectedPin:3,cutError:0,turn:0,door:0}},
      {title:'Raise one cut',instruction:'Start with cut 3 raised by 0.12 model units. Run the action, then inspect pin stack 3.',observe:'The key seats, but the lower pin crosses the boundary. Rotation and door opening remain blocked.',reset:true,cutaway:true,isolate:false,part:'pins',view:'side',values:{operation:0,insertion:0,selectedPin:3,cutError:0.12,turn:0,door:0}},
      {title:'Lower the same cut',instruction:'Start with cut 3 lowered by 0.12 model units. Run the action and inspect the same joint.',observe:'Now the upper driver bridges the boundary. Too low blocks the plug just as too high does.',reset:true,cutaway:true,isolate:false,part:'pins',view:'side',values:{operation:0,insertion:0,selectedPin:3,cutError:-0.12,turn:0,door:0}},
      {title:'Repair and use your key',instruction:'Start with the lowered cut and key out. Restore Change selected cut to zero, then run the opening action.',observe:'Restoring the shape aligns every joint and lets the same connected mechanism open the door.',reset:true,cutaway:true,isolate:false,part:'plug',view:'side',values:{operation:0,insertion:0,selectedPin:3,cutError:-0.12,turn:0,door:0}},
      {title:'Compare the blade limits',instruction:'Start with cut 1 requesting −0.20. Read Applied cut change, then run the opening action. Withdraw, select cut 4 and request +0.20 for comparison.',observe:'The first request applies only −0.145; the second applies +0.160. Both hit a blade limit. Once seated, the selected joint shows the actual change and blocks rotation.',reset:true,cutaway:true,isolate:false,part:'pins',view:'side',values:{operation:0,insertion:0,selectedPin:1,cutError:-.2,turn:0,door:0}},
      {title:'Withdraw along the slopes',instruction:'Start with the matching key seated and upright. Advance Close and relock, then compare the moving joint with the unchanged cut shape.',observe:'The selected joint rises and falls along the passing slopes while Applied cut change stays zero. With the key out, drivers obstruct the plug and the cut controls unlock.',reset:true,cutaway:true,isolate:false,part:'plug',view:'side',values:{operation:1,insertion:1,selectedPin:3,cutError:0,turn:0,door:0}},
    ],
    deeper: [
      ...
      {title:'Withdrawal needs aligned bores and slopes',body:'Returning the plug to zero lines up its pin bores with the shell. The key can then slide while sloping transitions lift and lower the pins. This single-edge teaching key demonstrates withdrawal without cuts on both blade edges; different key designs use different profiles. A turned plug keeps insertion disabled here.'},
    ],
    ...
    limits: 'This lesson edits one cut at a time in an enlarged pin-cylinder key. Cut heights are clamped to 0.025–0.500 model units; Applied cut change exposes this limit. The control is not a locksmith’s cutting scale. The blade cross-section is fixed, so keyway-profile compatibility is explained but not tested. Pin contact and cam travel use the parent cylinder model’s ideal geometry and prescribed motion; forces, wear, manufacturing clearances and other key technologies are not simulated.',
    sources: [{title:'Schlage: key cuts, bitting, and pin matching',url:'https://www.schlage.com/content/dam/sch-us/documents/pdf/installation-manuals/P513-325.pdf'},{title:'Linus Yale: divided pins, plug alignment and key withdrawal',url:'https://patents.google.com/patent/US48475A/en'}],
  ...
  'Zipper': {
    ...
    tryIt: [
      {title:'Fasten two separate sides',instruction:'Begin with the two sides completely apart. Run Fasten the jacket and follow alignment, pin insertion, then slider travel.',observe:'The pin enters through the lowered slider into the box before the tooth chain closes upward.',reset:true,cutaway:true,isolate:false,part:'garment',view:'front',values:{operation:0,alignment:0,insertion:0,closure:0,spread:.65,pullAngle:35}},
      {title:'Inspect the bottom connection',instruction:'The pin is aligned but only half inserted. Keep Look inside selected and isolation off. Move Seat the bottom pin to one.',observe:'The free pin tip moves toward the floor of the retaining box. Closing becomes available after full seating; the slider has stayed at the bottom.',reset:true,cutaway:true,isolate:false,part:'bottom-connector',view:'front',values:{operation:0,alignment:1,insertion:.5,closure:0,spread:.65,pullAngle:70}},
      {title:'Inspect a nested head',instruction:'Start with the bottom pin seated and the slider halfway up. Keep Look inside selected to compare the raised face and hollow underside of head 7 with its neighbor.',observe:'The head is a shaped solid. Its raised face enters the neighboring hollow while the two tapes remain connected at the bottom.',reset:true,cutaway:true,isolate:false,part:'head-7',view:'front',values:{operation:1,alignment:1,insertion:1,closure:.5,spread:.65,pullAngle:70}},
      {title:'Separate the jacket completely',instruction:'Start fastened. Run Separate the jacket. Watch all three stages instead of stopping as soon as the teeth open.',observe:'The slider reaches the box, the pin withdraws upward through it, and the right side moves away. The slider stays with the left side.',reset:true,cutaway:true,isolate:false,part:'garment',view:'front',values:{operation:1,alignment:1,insertion:1,closure:1,spread:.65,pullAngle:35}},
    ],
    deeper: [
      {title:'The guides act as inclined planes',body:'The sloping slider guides turn travel along the chain into a smaller sideways movement of each tooth. Like a wedge, an ideal guide can exchange a longer input movement for a larger sideways force: input work equals output work when losses are absent. Real zippers also lose energy to friction and fabric bending. This animation prescribes the path; it does not calculate a closing force or mechanical-advantage value.'},
    ...
    sources: [{title:'OpenStax: inclined planes, wedges and work',url:'https://openstax.org/books/physics/pages/9-3-simple-machines'},{title:'YKK: zipper parts and open-end operating sequence (pages 7 and 14)',url:'https://www.ykkindia.com/public/frontend/assets/images/catalogue/YKK-Zipper-Catalogue.pdf'},{title:'YKK: incomplete pin insertion and open-part damage',url:'https://ykkamericas.com/wp-content/uploads/2021/10/FasteningCatalogue_update20220120.pdf'},{title:'YKK: zipper structure and slider direction',url:'https://www.ykk.com/english/ykk/tech/01.html'},{title:'Sundback: nesting tooth projections and recesses',url:'https://patents.google.com/patent/US1219881A/en'}],
  ...
  'Nail clippers': {
    ...
    steps: [
      ...
      {title:'Press through the cam',body:'The finger acts on the long handle. Its rounded heel stays against the upper arm. Compare the perpendicular Finger moment arm and Cam moment arm readings. A longer finger arm lets the same effort produce a stronger cam push.'},
    ...
    deeper: [
      ...
      {title:'The displayed ratio comes from travel',body:'For this prescribed motion, the ideal edge-force ratio is the small vertical finger travel divided by the reduction in jaw gap. This is the lossless virtual-work comparison: finger force times finger travel equals edge force times gap reduction. It includes both arms closing. The separate moment-arm readings give actual distances to the finger and cam force lines at the current pose. Their ratio concerns the handle about its moving post, so it is not the whole clipper’s edge-force ratio.'},
  ...
  'Tweezers': {
    ...
    parts: [
      ...
      {name:'Small practice block',role:'Its fixed 4 mm width sets the contact gap. The chosen mass sets the weight to support.'},
      {name:'Receiving tray',role:'Supports the block at its starting height and catches it after release.'},
      {name:'Force arrows',role:'Blue arrows show inward finger effort, gold arrows show normal forces pressing on the block, and the orange arrow shows downward weight. Upward friction is calculated in Holding capacity.'},
    ],
    tryIt: [
      {title:'Grip, lift and release',instruction:'Use the starting 3 N setting and run Grip and lift the block. Then choose Release into the tray and run again.',observe:'The arms close against the block, the whole tool lifts it, and release opens the pads and returns the block to the tray.',reset:true,part:'system',view:'front',isolate:false,values:{operation:0,squeeze:0,fingerPosition:.55,effort:3,lift:0,mass:10}},
      {title:'Recover a weak squeeze',instruction:'Run with the fingers near the joined heel. When pickup stops, move Effort position to 0.85 without changing force, then run again.',observe:'At 0.30 the same effort leaves an 8.72 mm gap, wider than the 4 mm block. Moving the effort closer to the tips bends them farther and produces enough contact force to lift the block.',reset:true,part:'system',view:'front',isolate:false,values:{operation:0,squeeze:0,fingerPosition:.3,effort:3,lift:0,mass:10}},
      {title:'Touching is not enough',instruction:'Run with a 30 g block. The pads touch, but the starting grip cannot support its weight. Increase Force on each arm to 4 N, then run again.',observe:'The gap stays at 4 mm. Holding capacity starts at 0.134 N against a 0.294 N weight; 4 N effort raises capacity to 0.356 N and allows pickup without the tips penetrating the block.',reset:true,part:'system',view:'front',isolate:false,values:{operation:0,squeeze:0,fingerPosition:.55,effort:3,lift:0,mass:30}},
      {title:'Close empty tips',instruction:'Start with the open tool already above the tray and 6 N effort. Run the grip action. Use Return block to starting tray, then try the first experiment.',observe:'The raised pads meet at a zero gap, but grip force on the distant block stays zero. The block remains in the tray.',reset:true,part:'system',view:'front',isolate:false,values:{operation:0,squeeze:0,fingerPosition:.55,effort:6,lift:1,mass:10}},
    ],
    deeper: [
  ...
  'Window shade': {
    ...
    steps: [
      {title:'Cover the window',body:'Choose the requested coverage, check Lowering target, and run Lower and gently hold. The bottom rail moves down without stretching, cloth leaves the roll, and the spring gains relative turns.'},
    ...
    tryIt: [
      {title:'Cover the whole window',instruction:'Start raised and run Lower and gently hold. Compare the requested coverage with Lowering target and the actual stopped position.',observe:'The shade settles at holding position 7 and covers the whole window. Added spring winding reaches 1.75 turns.',values:{operation:0,coverage:1,release:1},reset:true,part:'system',view:'front',isolate:false},
      {title:'A gentle release catches again',instruction:'The shade starts held at about 63% coverage. Run the prepared tug with Gentle: catch again.',observe:'The catch moves off stop 4, then seats against it again. The shade stays about 63% covered with 1.00 added spring turn.',values:{operation:1,coverage:.6,release:0},initialState:{heldCoverage:.6},reset:true,part:'system',view:'front',isolate:false},
      {title:'A brisk release opens the window',instruction:'The shade starts held at about 78% coverage. Run the prepared tug with Brisk: rewind. Use Look inside to follow the spring and pawls.',observe:'The pawls stay clear, fabric wraps onto the roller and the window becomes uncovered. Added winding returns from 1.25 turns to zero.',values:{operation:1,coverage:.75,release:1},initialState:{heldCoverage:.75},reset:true,part:'system',view:'front',isolate:false},
      {title:'Lower farther from an existing stop',instruction:'Start held at about 48% coverage and request 90%. Run the next lowering action without first rewinding.',observe:'Fabric continues downward from stop 3 to stop 6, about 93% coverage. Added winding grows from 0.75 to 1.50 turns; the earlier extension is retained.',values:{operation:0,coverage:.9,release:0},initialState:{heldCoverage:.45},reset:true,part:'system',view:'front',isolate:false},
      {title:'Compare a request with its nearest stop',instruction:'Start raised and request 20% coverage. Check Lowering target, then run the lowering action.',observe:'The first holding stop covers about 16%, so the final window coverage differs from the requested fraction. The catch holds at a quarter-turn position.',values:{operation:0,coverage:.2,release:0},reset:true,part:'system',view:'front',isolate:false},
      {title:'A lower request cannot lift the shade',instruction:'Start held at about 78% coverage but request 20% with Lower and gently hold. Run that action.',observe:'The fabric makes a small seating movement and returns to stop 5. Lowering target stays 78%. Raising requires Tug and release with Brisk: rewind.',values:{operation:0,coverage:.2,release:0},initialState:{heldCoverage:.75},reset:true,part:'system',view:'front',isolate:false},
    ],
    deeper: [
      {title:'Holding occurs at angular stops',body:'Four ratchet faces and two opposed pawls provide successive holding positions. This model picks the nearest quarter-turn stop, except that a full-coverage request reaches the first stop covering the entire window. The available coverages are about 16, 32, 48, 63, 78, 93 and 100 percent. Lowering target shows the actual destination. If the shade is already lower than a new request, another lowering action cannot lift it. Use a tug and brisk release to rewind.'},
    ...
    sources: [{title:'US4674550: conventional fixed ratchet, rotating pawls and spring roller',url:'https://patents.justia.com/patent/4674550'},{title:'Select Blinds Canada: coiled spring, holding ratchet and spring tension',url:'https://www.selectblindscanada.ca/tech-tips-adjusting-spring-tension.html'}],
  ...
  'Ratchet': {
    ...
    steps: [
      ...
      {title:'Turn the permitted way',body:'Choose Turn by hand and move Hand turning stroke to the right. The sloping tooth face lifts the pawl as the wheel advances. Read Retained position for the actual wheel movement; the hand slider is only your input stroke.'},
    ...
    parts: [
      ...
      {name:'Elastic return band',role:'The visible stretched band biases the pawl toward its seating pin; other designs may use a spring or gravity.'},
      {name:'Fixed wheel axle',role:'Supports the wheel through its central bore while remaining secured to the frame.'},
      {name:'Pointer and fixed scale',role:'Show wheel position within one revolution; the readouts also count complete turns.'},
    ],
    tryIt: [
      {title:'Advance and hold',instruction:'Press Play. Watch the pointer advance three teeth, pass its stop slightly, and settle back.',observe:'The pointer stays at 90° because the pawl now touches a steep stopping face.',reset:true,part:'system',view:'front',isolate:false,values:{stroke:3,direction:1,release:0,hand:0}},
      {title:'Keep the progress',instruction:'This setup starts with three teeth already advanced and the pawl engaged. Press Play to request counterclockwise movement.',observe:'The 90° position is retained. The requested backward motion is blocked by the catch.',reset:true,initialState:{position:3},part:'system',view:'front',isolate:false,values:{stroke:3,direction:-1,release:0,hand:0}},
      {title:'Release and undo',instruction:'This setup starts at 90° with the pawl lifted clear and counterclockwise selected. Press Play to undo the three teeth.',observe:'The pointer returns three teeth. The same reverse request now succeeds because the pawl is clear.',reset:true,initialState:{position:3},part:'system',view:'front',isolate:false,values:{stroke:3,direction:-1,release:1,hand:0}},
      {title:'Finish a complete turn',instruction:'Start with six teeth already advanced. Run six more and compare the pointer with Total turns.',observe:'The wheel reaches twelve teeth and 360°. Its pointer returns to the same direction as at reset, but the retained count is one full turn.',reset:true,initialState:{position:6},part:'system',view:'front',isolate:false,values:{stroke:6,direction:1,release:0,hand:0}},
      {title:'Count past one turn',instruction:'Start at twelve teeth and run three more. Compare this result with Advance and hold.',observe:'The pointer again points at the 90° position on the scale, but the wheel has reached fifteen teeth, 450° and 1.25 turns.',reset:true,initialState:{position:12},part:'system',view:'front',isolate:false,values:{stroke:3,direction:1,release:0,hand:0}},
      {title:'Feel the one-way stop',instruction:'Turn by hand starts at three teeth with the pawl engaged. Move Hand turning stroke from 0 to 1, then back to -1. Playback is disabled while you use the hand control.',observe:'The rightward stroke advances the wheel to four teeth. The leftward request stops at that tooth face, so the wheel keeps its progress while your input returns.',reset:true,initialState:{position:3},part:'system',view:'front',isolate:false,values:{stroke:3,direction:0,release:0,hand:0}},
      {title:'Turn both ways with the catch lifted',instruction:'Start at the same three-tooth position with the pawl clear. Move Hand turning stroke from 0 to 1, then to -1.',observe:'The wheel follows both requests: first four teeth, then two. The two-tooth return succeeds because no pawl blocks it.',reset:true,initialState:{position:3},part:'system',view:'front',isolate:false,values:{stroke:3,direction:0,release:1,hand:0}},
    ],
    deeper: [
      ...
      {title:'A driving pawl has another job',body:'An oscillating lever can carry a driving pawl that pushes a wheel forward. A separate holding pawl prevents the wheel from slipping back as the lever returns. Here your selected input supplies the turn, and the visible fixed-pivot pawl provides the holding constraint.'},
    ...
    limits: 'This enlarged principle demonstrator uses prescribed automatic strokes or incremental hand-input requests constrained by tooth contact. The hand slider is an input stroke, not an absolute wheel-position setting. Scene dragging still moves or rotates the view. An illustrative elastic band returns the catch. It does not calculate forces, tooth stress, friction, wear, release under load or safe holding capacity. The release setting places the pawl in its selected pose; it does not simulate the hand operating a release lever. The Window shade lesson applies the same holding principle with a fixed ratchet and moving pawls.',
    sources: [{title:'Carnegie Mellon University: driving and holding ratchet mechanisms',url:'https://www.cs.cmu.edu/~rapidproto/mechanisms/chpt8.html'},{title:'Shade-roller patent: ratchet and pawl holding mechanism',url:'https://patents.justia.com/patent/4674550'}],
  ...
  'Electric bell': {
    ...
    tryIt: [
      {title:'Ring the bell',instruction:'Press Play with the normal contact. Select Sound on if you want to hear the impacts.',observe:'Several strikes follow one held button press. The contacts repeatedly open and close, then the button releases and the bell stops.',reset:true,part:'system',view:'front',isolate:false,values:{voltage:3,holdTime:.35,contact:0,sound:0}},
      {title:'Leave a gap in the circuit',instruction:'Set up the held-open contact, then press Play. Inspect the gap at the screw.',observe:'The button still moves, but no coil current builds and the hammer does not strike.',reset:true,part:'system',view:'front',isolate:false,values:{voltage:3,holdTime:.35,contact:1,sound:0}},
      {title:'Bypass the interrupter',instruction:'Press Play with the blue jumper in place. Watch what happens after the first strike.',observe:'The jumper keeps supplying the coils when the contact opens. A steady pull holds the armature near the gong instead of sustaining repeated ringing.',reset:true,part:'system',view:'front',isolate:false,values:{voltage:3,holdTime:.35,contact:2,sound:0}},
      {title:'Try a weak battery',instruction:'Keep the normal contact and compare the weaker 1.5 V supply with 3 V.',observe:'Current flows, but this weaker supply cannot drive the hammer far enough to strike. A closed circuit alone does not guarantee enough motion.',reset:true,part:'system',view:'front',isolate:false,values:{voltage:1.5,holdTime:.35,contact:0,sound:0}},
      {title:'Press briefly',instruction:'Use the normal 3 V circuit and a 0.20 s press, then play. Watch the button release and the remaining spring return.',observe:'This model gives two strikes. Time since press continues after the button rises until the armature comes to rest.',reset:true,part:'system',view:'front',isolate:false,values:{voltage:3,holdTime:.2,contact:0,sound:0}},
      {title:'Hold the button longer',instruction:'Keep the same 3 V circuit but hold for 0.80 s. Compare the result with Press briefly.',observe:'The internal switch keeps cycling during one continuous external press, producing nine strikes in this model. Longer pressing allows more cycles; it does not make the gong tone lower.',reset:true,part:'system',view:'front',isolate:false,values:{voltage:3,holdTime:.8,contact:0,sound:0}},
    ],
    deeper: [
      ...
      {title:'Motion and current take time',body:'The coils resist a sudden change in current, and the moving armature has inertia. Those delays matter: the hammer can keep moving after the contact opens. The spring and damping then affect its return. Here L/R = 0.08 H / 10 Ω = 8 ms sets the current rise, while the open-contact decay is prescribed as 0.5 ms. Time since press reports physical model time rather than the slower screen time.'},
    ...
    limits: 'This representative bell uses a hinged armature and torsion return spring. A simplified current and rotational-motion model determines the strikes. Motion is shown at 0.12 times real speed, about one eighth speed. Coil resistance, inductance, magnetic-force coefficient, saturation, damping and current-decay time are illustrative, not calibrated ratings. Contact preload is shown geometrically; its extra torque, arcing, magnetic hysteresis and acoustic resonance are not solved. The audible tone is synthesized, and the blue field and sound rings are explanatory symbols.',
    sources: [{title:'OpenStax University Physics: RL current and time constants',url:'https://openstax.org/books/university-physics-volume-2/pages/14-4-rl-circuits'},{title:'Historical self-interrupting bell patent (separate-clapper variant)',url:'https://patents.google.com/patent/US1937403A/en'},{title:'Schoolphysics: self-interrupting electric bell',url:'https://schoolphysics.co.uk/age11-14/Electricity%20and%20magnetism/Electromagnetism/text/Electric_bell/index.html'}],
