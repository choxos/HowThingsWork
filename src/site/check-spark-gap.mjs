...
try{
 await page.goto(`${process.env.SITE_URL||'http://127.0.0.1:4177/'}#machine/spark-gap`);await page.getByRole('heading',{name:'Spark gap',exact:true}).waitFor();await page.locator('.daily-readings').waitFor();
 ...
 for(const [key,next] of [['voltage','9'],['rpm','120'],['width','2.5']]){await inspect(1);await number(key).fill(next);near(await value('Observation progress'),0);near(await value('Plug A delivered energy'),0);near(await value('Plug A spark episodes'),0);}
