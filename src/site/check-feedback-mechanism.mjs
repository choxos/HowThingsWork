...
import {sampleFeedback,feedbackConstants} from './feedback-mechanism-model.js';
// The lesson gives levels in millimeters; the model takes meters.
const atLesson=(values,time)=>sampleFeedback({...values,level:values.level/1000},time);
...
async function response(v){
 ...
 // A full tank holds its area times the overflow rim height, in liters.
 assert.ok(s['Stored water']>=0&&s['Stored water']<=feedbackConstants.tankArea*feedbackConstants.rim*1000+.05);for(const key of ['Admitted water','Delivered water','Overflowed water','Unmet demand'])assert.ok(s[key]>=0);
 return s;
}
try{
 ...
 for(let i=0;i<lesson.tryIt.length;i++){
  ...
  outcomes.push(await response(v));if(i===0)assert.ok((await reading('Your result')).startsWith(`${Number((atLesson(v,240).level*1000).toFixed(1))} mm · `));await shot('experiment-'+i);console.log(`PASS experiment ${i+1}: ${lesson.tryIt[i].title}`);
 }
 ...
 near(outcomes[0]['Water level'],atLesson(lesson.tryIt[0].values,240).level*1000,.05);near(outcomes[0]['Inlet flow'],atLesson(lesson.tryIt[0].values,240).inletFlow*1000,.0001);near(outcomes[1]['Water level'],180,.002);near(outcomes[1]['Delivered water'],0);
 near(outcomes[2]['Water level'],300);near(outcomes[2]['Overflowed water'],atLesson(lesson.tryIt[2].values,240).overflowVolume*1000,.005);assert.ok(outcomes[2]['Overflow now']>0);
 for(const i of [3,7]){near(outcomes[i]['Stored water'],atLesson(lesson.tryIt[i].values,240).storedVolume*1000,.005);near(outcomes[i]['Delivered water'],4.8);near(outcomes[i]['Inlet flow'],0);}
 ...
 await page.setViewportSize({width:390,height:844});await preset(4);await inspect(4);await page.evaluate(()=>window.scrollTo(0,0));assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await shot('mobile-capacity');near(await value('Stored water'),0);await preset(2);await inspect(4);await shot('mobile-overflow');near(await value('Overflowed water'),atLesson(lesson.tryIt[2].values,240).overflowVolume*1000,.005);assert.deepEqual(errors,[]);
