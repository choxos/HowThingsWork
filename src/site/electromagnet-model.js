...
export function createElectromagnetModel(){
 ...
 const forces=part('forces','Forces on the load','Magnetic attraction acts upward and weight acts downward. The tray pushes upward on a resting plate; the magnet face pushes downward on a captured plate. Arrow lengths use one shared force scale.',[0,0,0],system);
 ...
 const result=finish(v=>{
  ...
  const reaction=held?Math.max(0,fieldState.force-fieldState.required):0,trayReaction=loadY===0?Math.max(0,fieldState.weight-fieldState.force):0,powered=stage!=='ready'&&v.operation===0;
  ...
  reactionArrow.setDirection(new THREE.Vector3(0,trayReaction>0?1:-1,0));
  const forceScale=.75/Math.max(fieldState.force,fieldState.weight,reaction,trayReaction);for(const [arrow,value,x] of [[magneticArrow,fieldState.force,1.29],[weightArrow,fieldState.weight,1.46],[reactionArrow,reaction+trayReaction,1.63]]){arrow.visible=value>1e-8;arrow.position.set(x,load.position.y,1.0);const length=Math.max(1e-8,value*forceScale);arrow.setLength(length,Math.min(.08,length*.35),Math.min(.045,length*.22));}
  ...
  return {state:{current,loadY,loadVelocity,hoistY,hoistVelocity,hoistAcceleration,elapsed,stage,complete,blocked,held,contact,captured,falling,powered,force:fieldState.force,weight:fieldState.weight,requiredForce:fieldState.required,normalReaction:reaction,trayReaction,gap:fieldState.gap,separation:fieldState.separation,forceValid:fieldState.valid,magneticCircuitValid:fieldState.valid&&v.material===0,flux:fieldState.valid&&v.material===0?fieldState.flux:null,centralFluxDensity:fieldState.valid&&v.material===0?fieldState.flux/centralArea:null,returnFluxDensity:fieldState.valid&&v.material===0?fieldState.flux/returnArea:null,loadHeight:loadY,hoistHeight:hoistY,hoistAtTarget,leadLength,wireSpan,wireBow,currentDirection:Math.abs(current)<.005?0:sign},readings:[
   r('Your result',status,'Pickup requires enough attraction to close the initial gap. The hoist waits for actual contact.'),
   r('Coil current',`${current.toFixed(2)} A`,'Regulated driver response; sign sets polarity. Motion and current response run ten times slower for inspection.'),
   r('Magnetic attraction',v.material===1?'0 N · no ferromagnetic load':fieldState.valid?`${fieldState.force.toFixed(2)} N`:'Not estimated beyond the 3 mm capture gap','Upward pull on the iron load. Closing both pole gaps reduces reluctance and increases attraction.'),
   r('Load weight',`${fieldState.weight.toFixed(2)} N`,'Downward gravitational force: load mass × 9.81 m/s².'),
   r('Required lifting force',`${fieldState.required.toFixed(2)} N`,'Mass × (gravity + hoist acceleration). Once captured, attraction must meet this requirement.'),
   r('Contact force',reaction>0?`${reaction.toFixed(2)} N · magnet face pushes down`:trayReaction>0?`${trayReaction.toFixed(2)} N · tray pushes up`:'0.00 N · no support force','Gold arrow: the tray supports the resting load; the magnet face balances excess attraction when the load is held.'),
   r('Magnetic gap',`${(fieldState.gap*1000).toFixed(3)} mm`,'Gap at each pole face, including the 0.2 mm nonmagnetic surface layer. Air separation closes to zero at contact.'),
   r('Central pole flux density',v.material===0&&fieldState.valid?`${(fieldState.flux/centralArea).toFixed(3)} T`:v.material===1?'Not estimated · nonmagnetic return path':'Not estimated · outside capture gap','Field magnitude per unit area in the center pole, capped at 1.4 T. An open return path still has a field, but this closed-circuit estimate does not apply.'),
   r('Pole direction',Math.abs(current)<.005?'Unenergized · remanence omitted':sign>0?'Center N · outer ring S':'Center S · outer ring N','Directions refer to the lower pole faces. Reversing current exchanges poles; iron is still attracted.'),
   r('Load above tray',`${(loadY*1000).toFixed(2)} mm`,'Rise from the starting tray. A completed 40 mm hoist lift gives 42 mm of load rise after closing the initial 2 mm gap.'),
   r('Hoist travel',`${(hoistY*1000).toFixed(2)} / ${v.height} mm`,'Current height / selected lift target. Release holds the hoist at its existing height while the load falls.'),
   r('Model limit','Narrow-gap teaching model, not a lifting rating','Uniform fields, capped flux density, idealized contacts, no fringing, hysteresis or eddy-current model.'),
  ]};
 });
 ...
 field.userData.explosionExcluded=forces.userData.explosionExcluded=true;
 result.catalogParts=result.parts.filter(p=>!['system','field','forces'].includes(p.id));
