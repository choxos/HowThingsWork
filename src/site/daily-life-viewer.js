...
export function mountDailyLifeViewer(host,name,providedModel,{onExit}={}) {
  ...
  let overviewZoom=1.15,followPosition=null;
  ...
  function stop(){playing=false;model.playback?.setPlaying?.(false);cancelAnimationFrame(frame);syncPlaybackButton();}
  ...
  function reset(initialState){stop();phase=0;model.reset?.(initialState);model.animate?.(0);apply(Object.fromEntries(model.controls.map(control=>[control.key,control.initial])));if(selected===model.resultPart?.id){isolated=false;options.querySelector('[data-isolate]').checked=false;selectPart(null);}}
