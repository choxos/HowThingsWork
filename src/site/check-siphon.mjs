...
import {siphonLesson as lesson} from './siphon-lesson.js';
...
export async function checkSiphon(page,{base,dir,prefix='local',widths=[1440,390],playback=true}){
 await mkdir(dir,{recursive:true});const model=createSiphonModel(),observations=[],playbackCases=[];
 ...
 try{
  for(const width of widths){
   await page.setViewportSize({width,height:width===390?844:1000});await page.goto('about:blank');await page.goto(base+'#machine/siphon');await page.locator('[data-control="fault"]').waitFor();await page.evaluate(()=>document.fonts.ready);
   assert.equal(await page.locator('h1').innerText(),'Siphon');assert.equal(await page.locator('[data-control]').count(),5);assert.equal(await page.locator('[data-action]').count(),model.actions.length);assert.equal(await page.locator('[data-experiment]').count(),lesson.tryIt.length);
  ...
  const result={passed:true,observations,playbackCases};await writeFile(`${dir}/${prefix}-siphon-browser.json`,JSON.stringify(result,null,2)+'\n');return result;
