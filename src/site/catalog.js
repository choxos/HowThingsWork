...
const modeledNames = new Set(['Commutator','Direct-current motor','Electromagnet','Electric-horn moving iron bar','Horn make-and-break contacts','Electric horn','Vibrating horn diaphragm',...neighborhoodCatalog.places.flatMap(place=>place.featured),...allEntries.filter(entry=>entry.place==="home").map(entry=>entry.name)]);
