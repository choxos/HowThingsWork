...
if(process.env.MODEL_ONLY!=='1'){
 ...
 try{
  for(const width of [1440,390]){
   ...
   await page.getByRole('button',{name:'Back to The workshop',exact:true}).click();await page.getByRole('heading',{name:'The workshop',exact:true}).waitFor();assert.equal(await page.locator('button[data-entry="microchip-deceleration-sensor"]').count(),0);await page.locator('button[data-entry="crash-sensor"]').click();await landed();await capture('room-opening');await page.goBack();await page.getByRole('heading',{name:'The workshop',exact:true}).waitFor();await page.goForward();await landed();cases.push({width,entry:'room',backForward:true});
   await page.goto(base+'#list');await page.getByRole('heading',{name:'Explore the finished collection.',exact:true}).waitFor();await page.locator('#catalog-search').fill('Microchip deceleration sensor');const row=page.locator('tr').filter({has:page.locator('[data-entry="crash-sensor"]')});assert.equal(await row.locator('.catalog-components [data-entry="microchip-deceleration-sensor"]').count(),0);assert.equal(await page.locator('td>button[data-entry="microchip-deceleration-sensor"]').count(),0);await row.locator('[data-entry="crash-sensor"]').click();await landed();await capture('catalog-opening');await page.goBack();await page.getByRole('heading',{name:'Explore the finished collection.',exact:true}).waitFor();cases.push({width,entry:'catalog',canonical:true,back:true});
