import {groupCatalogEntries,catalogMachineComponents,partHref,hasCatalogPart} from './catalog-hierarchy.js';
...
const roomPositions=[[22,68],[79,32],[60,65],[23,32],[62,35],[81,68],[46,72],[46,62],[85,53],[91,43]];
...
export function mountHouse(host,route,catalog,{part:requestedPart}={}) {
 ...
 const roomFamilies=groupCatalogEntries(entries.filter(entry=>roomGroups.some(group=>group.id===entry.group))).map(family=>({...family,components:catalogMachineComponents(family.components)}));
 const tile=roomTiles[room],previews=roomFamilies.map(({entry})=>({id:entry.id,name:entry.name}));
 const roomScene=tile?`<section class="zoom-scene house-zoom-scene house-room-frame" aria-label="Illustrated ${esc(room)}"><div class="house-zoom-layer house-room-scene">${roomBackdrop(room)}${Array.from({length:Math.ceil(previews.length/3)},(_,page)=>`<div class="house-room-tray" data-spatial-page="${page}" ${page?'hidden':''}>${previews.slice(page*3,page*3+3).map((entry,i)=>`<a class="${['dishwasher','faucet','water-meter','toilet-tank'].includes(entry.id)?'house-installed':''}" style="--slot-left:${4+i*31}%" data-zoom-target="${esc(entry.name)}" data-machine="${entry.id}" href="#machine/${entry.id}">${thumbnail}<span>${esc(entry.name)} →</span></a>`).join('')}</div>`).join('')}</div></section>`:'';
 host.innerHTML=`<div class="plan-layout"><section class="plan-intro"><span class="badge">Inside the house</span><h1 tabindex="-1">${esc(room)}</h1><p class="intro">${esc(roomNotes[room])}</p><p class="intro">Point at an object and zoom closer to look inside.</p><a class="primary" href="#list">All machines & ideas</a><p class="plan-note">Choose an object in the illustration, or browse every object below.</p></section><div class="plan-art">${room==='Doors and daily life'?`<div class="zoom-scene house-zoom-scene house-room-frame"><div class="house-zoom-layer house-hall">${roomBackdrop(room)}<a data-zoom-target="Cylinder lock" style="--x:55%;--y:47%" href="#machine/cylinder-lock">Cylinder lock</a><a data-zoom-target="Coat zipper" style="--x:17%;--y:39%" href="#machine/zipper">Coat zipper</a><a data-zoom-target="Electric bell" style="--x:48%;--y:12%" href="#machine/electric-bell">Electric bell</a><a data-zoom-target="Window shade" style="--x:80%;--y:15%" href="#machine/window-shade">Window shade</a><a data-zoom-target="Nail clippers" style="--x:79%;--y:59%" href="#machine/nail-clippers">Nail clippers</a></div></div>`:roomScene}</div></div>${crumbs(room)}<details class="house-directory"><summary>Browse every object</summary><section class="house-object-grid" aria-label="Discoveries">${roomFamilies.map(({entry,components})=>`<article class="house-object" data-machine="${entry.id}"><a href="#machine/${entry.id}">${thumbnail}<h2>${esc(entry.name)}</h2></a><p>${esc(allLessons[entry.name]?.simple||'Open this object to follow its mechanism.')}</p>${components.length?`<div class="house-component-links" aria-label="Smaller machines in ${esc(entry.name)}">${components.map(part=>`<a href="#machine/${part.id}">${esc(part.name)}</a>`).join('')}</div>`:''}</article>`).join('')}</section></details>`;
