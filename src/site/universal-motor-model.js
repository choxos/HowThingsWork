...
export function createUniversalMotorModel(){
 ...
 const stator=part('stator','Laminated stator and two pole shoes','The joined iron yoke returns flux between the wound poles. Eight representative sheet layers illustrate lamination; actual cores use many thin insulated sheets.',[0,0,0],assembly);
 ...
 for(let i=0;i<8;i++){
  const z=-.7+i*.2,color=i%2?'metal':'ink';
  for(const [size,position] of [[[.2,2.34,.196],[-1.62,0,z]],[[.2,2.34,.196],[1.62,0,z]],[[3.04,.18,.196],[0,1.08,z]],[[3.04,.18,.196],[0,-1.08,z]],[[.6,.44,.196],[-1.25,0,z]],[[.6,.44,.196],[1.25,0,z]]]){const mesh=box(size,position,color,yoke);mesh.updateMatrix();mesh.geometry.applyMatrix4(mesh.matrix);yokeGeometries[i%2].push(mesh.geometry);yoke.remove(mesh);}
 }
 for(let i=0;i<2;i++){const mesh=new THREE.Mesh(mergeGeometries(yokeGeometries[i]),new THREE.MeshToonMaterial({color:i?0xa6b59f:0x99a98f}));yoke.add(mesh);yokeGeometries[i].forEach(g=>g.dispose());}
 ...
 const sleeve=part('commutator-sleeve','Insulating commutator sleeve','An annular sleeve separates all three copper segments from the shaft. The central bore fits the steel shaft.',[0,0,0],commutator);
 const sleeveProfile=[[.065,-.1],[.21,-.1],[.21,.1],[.065,.1],[.065,-.1]].map(([x,y])=>new THREE.Vector2(x,y));
 const sleeveMesh=new THREE.Mesh(new THREE.LatheGeometry(sleeveProfile,96),new THREE.MeshToonMaterial({color:0xf0dfaf}));sleeveMesh.rotation.x=Math.PI/2;sleeve.add(sleeveMesh);
 ...
 for(let j=0;j<4;j++){const blade=part('fan-blade-'+j,'Pitched fan blade '+(j+1),'The blade root enters the hub attached to the continuous shaft.',[0,0,0],fan);blade.rotation.z=j*Math.PI/2;const mesh=box([.25,.62,.055],[0,.38,0],'leaf',blade);mesh.rotation.y=.35;}
 ...
 const result=finish(v=>{
  ...
  return {state:{theta,omega,rpm,elapsed,actionTime,sourceTime,travel,stage,complete,blocked,powered,voltage,...e,fanTorque,fanPower,ribbonBend,kineticEnergy:.5*C.inertia*omega**2},readings:[r('Your result',status,'Powered playback pauses a moving state. Disconnect finishes only when stored motion has decayed to rest.'),r('Shaft speed',`${rpm.toFixed(1)} rpm`,'Rotations per minute, calculated from torque, inertia and opposing load. Supply frequency does not set this speed.'),r('Source voltage',`${voltage.toFixed(3)} V`,v.source===1?'Slow cosine AC; setting is peak, not RMS.':'Signed DC voltage.'),r('Series current',`${e.seriesCurrent.toFixed(3)} A`,'Both stator windings carry this total brush current.'),r('Field direction',Math.abs(e.fieldB)<1e-6?'No field · current is zero':e.fieldB>0?'Left N · right S':'Left S · right N','Pole identities reverse with the series current. Their positions stay fixed.'),r('Field strength',`${e.fieldB.toFixed(3)} T`,'Assumed B = 0.3 × series current.'),r('Armature branch currents',e.branchCurrents.map((i,j)=>`${j}: ${i.toFixed(3)} A`).join(' · '),'Positive branch j runs from Sj to the next segment.'),r('Brush contacts',pair,'P and Q are fixed brushes. S0–S2 turn with the shaft; a plus sign means one brush touches two segments.'),r('Commutation',e.shortedCoils.length?'Brush shorts branch '+e.shortedCoils.join(', '):'Two parallel armature paths','A shorted winding can still carry induced current.'),r('Driving torque',`${(e.torque*1000).toFixed(3)} mN·m`,'Sum of all three winding torques, before bearing friction and fan drag. Both field and winding-current signs affect it.'),r('Fan power',`${(fanPower*1000).toFixed(2)} mW`,'Illustrative aerodynamic load, not a fan rating.'),r('Rotor travel',`${travel.toFixed(2)} turns`,'Accumulated shaft rotation since the experiment was reset; includes the powered preparation of a coast.'),r('Simulated time',`${elapsed.toFixed(2)} s`,'Motion is shown twice as slowly.'),r('Model limit','Reduced three-coil ring · zero inductance','Slow AC only; assumed resistance, linear field, inertia, friction and fan drag. Not mains-frequency performance.')]};
 });
 ...
 for(const guide of [field,...fieldArrows,...coilArrows,...forceArrows])guide.userData.explosionExcluded=true;
 result.catalogParts=result.parts.filter(p=>!['system','field'].includes(p.id));
