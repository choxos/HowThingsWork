...
try{
 ...
 // Play on a finished run repeats the experiment that was configured, so the
 // pawl has to be put back to holding before it can hold again.
 await page.getByRole('combobox',{name:'Holding pawl',exact:true}).selectOption('0');await page.getByRole('combobox',{name:'Requested direction',exact:true}).selectOption('1');
 await play();await wait('Pawl holding');assert.match(await page.locator('.daily-readings').textContent(),/90.0°/);
