...
export function createClipperModel(){
 ...
 const result=finish(v=>{
  ...
  const fingerMoment=Math.abs(fingerX-g.pivotX),camMoment=Math.abs((g.contactX-g.pivotX)*g.normal.y-(g.contactY-g.pivotY)*g.normal.x);
  return {state:{cut,drop,complete,blocked,forceRatio:ratio,outputForce:v.effort*ratio,jawGap:Math.max(0,g.gap),contactSqueeze,contact:g},readings:[
   r('Your result',complete?'Nail edge clipped · fragment in tray':cut?'Clipped · release to clear the fragment':blocked?'Strip held · more leverage or effort needed':'Fresh strip · ready to clip','A complete action cuts the visible bridge, reopens the jaws and moves the fragment into the tray. The strip resistance is a teaching threshold.'),
   r('Ideal force ratio',ratio.toFixed(2)+' ×','Small vertical finger travel divided by total jaw-gap reduction. The changing handle angle changes this ratio through the stroke; both spring arms move. Spring work and friction are excluded.'),
   r('Ideal edge force now',(v.effort*ratio).toFixed(1)+' N','Finger force multiplied by the current ideal travel ratio. This is an available-force estimate, not a measured force on an unloaded or already clipped strip.'),
   r('At first contact',contactForce.toFixed(1)+' N available · '+v.resistance+' N required','The clipping rule uses the ideal force when the gap first reaches the 0.050-model-unit strip thickness. Compare this with the chosen resistance; the open-pose force can be smaller.'),
   r('Jaw gap',Math.max(0,g.gap).toFixed(3)+' model units','Vertical separation of the two opposed cutting tips. First strip contact is 0.050; a completed return restores the original open gap. These enlarged units are not millimeters.'),
   r('Finger moment arm',fingerMoment.toFixed(3)+' model units','Perpendicular distance from the handle pivot to the vertical line of finger effort, measured at the blue marker. Moving the finger farther out increases this distance.'),
   r('Cam moment arm',camMoment.toFixed(3)+' model units','Perpendicular distance from the handle pivot to the cam contact-normal force line. Comparing these two moment arms describes the handle alone; the displayed force ratio also includes the moving post and both bending blades.'),
   r('Next action',complete?'Load a fresh strip to compare another setting.':blocked?'Move the finger farther along the handle or increase effort, then run again.':cut?'Release the handle or run the return.':'Run the clipping action, or press the lever yourself.','Play presses, clips and releases. A weak input pauses at contact for a change in leverage or effort. Load a fresh strip keeps your force and finger settings.')
  ]};
 },{animated:true});
