...
export function createRatchetModel(){
 ...
 control('direction','Wheel input',-1,1,1,1,'','Run a clockwise or counterclockwise stroke, or choose Turn by hand to explore tooth contact with the hand slider.',[{value:1,label:'Advance clockwise'},{value:-1,label:'Reverse counterclockwise'},{value:0,label:'Turn by hand'}]);
 control('stroke','Requested travel',1,6,1,3,'teeth','One tooth is 30°. Each automatic action starts at the current wheel position.',undefined,{enabledWhen:v=>v.direction!==0});
 control('hand','Hand turning stroke',-1,1,.05,0,'teeth','In Turn by hand mode, drag right to request clockwise movement and left to test reverse. This is your input stroke, not the wheel position; repeated strokes can accumulate turns.',undefined,{enabledWhen:v=>v.direction===0});
 ...
 let angle=0,actionStartAngle=0,beta=0,checkpoint=0,stage='ready',complete=false,clock=0,target=0,stop=0,lastClock=0,accumulator=0,wasReleased=false;
 ...
 const result=finish(v=>{
  ...
  const blocked=stage==='blocked',atStop=!v.release&&Math.abs(angle-checkpoint)<1e-9&&Math.abs(beta)<1e-9,held=stage==='held'||v.direction===0&&atStop;
  return {state:{angle,beta,checkpoint,stage,complete,blocked,held,position:angle/step,clearance:clearance(beta)},readings:[
   r('Your result',blocked?'Reverse blocked · advanced position retained':v.direction===0?(v.release?'Hand turning · pawl clear in both directions':held?'Hand turning · pawl holds this stop':'Hand turning · pawl follows the tooth'):held?'Pawl holding · advanced position retained':stage==='free'?'Pawl released · position can move either way':stage==='settle'?'Settling back to the next stop':stage==='move'?'Turning the wheel':v.direction===-1&&!v.release?'Ready · pawl blocks counterclockwise; lift it clear to reverse':'Ready · choose direction and press Play'),
   r('Retained position',(angle/step).toFixed(2)+' teeth','Actual wheel position relative to reset, including complete turns. It can move while the pawl is lifted; twelve teeth make one turn.'),
   r('Wheel rotation',(angle*180/Math.PI).toFixed(1)+'°','Total clockwise angle from reset, not just the pointer direction within one revolution. Counterclockwise positions are negative.'),
   r('Total turns',(angle/(Math.PI*2)).toFixed(3)+' turns','The fixed pointer scale repeats every twelve teeth, but this count continues past one full turn.'),
   r('Last holding stop',v.release?'Pawl lifted':(checkpoint/step).toFixed(0)+' teeth','With the pawl engaged, reverse movement can settle to this last passed tooth face but cannot go beyond it.'),
   r('Pawl',v.release?'Lifted clear':atStop?'Toe against stopping face':beta===0?'At seating pin':'Riding tooth ramp','A ramp lifts the catch; a stopping face and fixed seating pin block reverse travel. The release setting lifts the catch away.'),
   r('Pawl lift',(-beta*180/Math.PI).toFixed(1)+'°','Angular lift from the seating pin, measured from the actual pawl pose. Contact geometry sets the pose; force and spring dynamics are not calculated.'),
   r('Next comparison',v.direction===0?'Move Hand turning stroke right and left. Compare engaged and lifted pawl.':'Change direction or lift the pawl, then press Play to act from this position.','Automatic playback is paused in Turn by hand mode. Choose an automatic direction to run a stroke from the position you reached.')
  ]};
 });
 const render=result.update;
 result.update=next=>{const before=result.getState().values;const readings=render(next),v=result.getState().values;if(Object.keys(before).some(k=>before[k]!==v[k])){stage='ready';complete=false;clock=0;accumulator=0;
  if(v.direction===0&&v.hand!==before.hand){const delta=(v.hand-before.hand)*step,requested=angle+delta;angle=v.release?requested:Math.max(checkpoint,requested);if(!v.release&&delta>0)checkpoint=Math.floor(angle/step+1e-9)*step;stage=angle>requested+1e-9?'blocked':'hand';}
  return render();}return readings;};
 function tick(dt){const v=result.getState().values;if(stage==='ready'){actionStartAngle=angle;clock=0;if(v.direction===-1&&!v.release){stop=checkpoint;stage=angle>stop+1e-9?'settle':'blocked';}else{stop=v.release?angle+v.direction*v.stroke*step:(Math.floor(angle/step+1e-9)+v.stroke)*step;target=stop+(v.release?0:.12);stage='move';}}
 ...
 function advance(dt){if(!Number.isFinite(dt)||dt<=0||complete||result.getState().values.direction===0)return render();accumulator+=dt;while(accumulator>=.01-1e-9&&!complete){accumulator-=.01;tick(.01);}if(complete)accumulator=0;return render();}
 result.reset=(initialState={})=>{if(initialState.angle!==undefined&&!Number.isFinite(initialState.angle))throw new RangeError('Invalid initial angle');if(initialState.position!==undefined&&!Number.isFinite(initialState.position))throw new RangeError('Invalid initial position');angle=initialState.angle??(initialState.position===undefined?0:Math.trunc(initialState.position)*step);actionStartAngle=angle;beta=0;checkpoint=Math.floor(angle/step+1e-9)*step;accumulator=0;wasReleased=false;stage='ready';complete=false;clock=0;lastClock=0;return render(result.defaults);};
 ...
 result.playback={label:'Run selected action',stepLabel:'Advance one step',description:'Move the wheel through the requested teeth, then watch the holding result.',advance,step:()=>advance(.2),complete:()=>complete,blocked:()=>result.getState().values.direction===0,blockedReason:'Use the hand slider, or choose an automatic Wheel input to run a stroke.'};
