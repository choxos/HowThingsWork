...
const evidence=new URL(process.env.MOTOR_ROTOR_EVIDENCE||'../../documentation/audit/evidence/motor-rotor/',import.meta.url);await mkdir(evidence,{recursive:true});
...
try{
 ...
 assert.match(await page.locator('.daily-part-detail').textContent(),/Three-coil laminated rotor/);assert.equal(await page.locator('[data-isolate]').isChecked(),false);await page.locator('canvas').screenshot({path:new URL('opening-rear.png',evidence).pathname});
