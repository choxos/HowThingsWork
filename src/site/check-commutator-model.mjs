...
function verify(model){const s=model.getState(),readout=s.readings.find(r=>r.label==='Brush pair')?.value;
 const copperAt=angle=>{const local=(angle-s.theta)%(2*Math.PI),a=(local+2*Math.PI)%(2*Math.PI);return a>s.gapHalf&&a<Math.PI-s.gapHalf?'A':a>Math.PI+s.gapHalf&&a<2*Math.PI-s.gapHalf?'B':null;};
 const contacts=angle=>new Set([-s.brushHalf,0,s.brushHalf].map(offset=>copperAt(angle+offset)).filter(Boolean));const plus=contacts(0),minus=contacts(Math.PI);assert.ok(plus.size<=1&&minus.size<=1,'no brush bridges both halves');assert.equal(s.contact,plus.size===1&&minus.size===1,'current availability matches actual copper/brush angles');
 if(s.contact){const a=[...plus][0],b=[...minus][0];assert.notEqual(a,b);assert.equal(readout,`+x: ${a} · −x: ${b}`,'readout identifies the geometric segment pair');assert.ok(Math.abs(s.coilCurrent-(a==='A'?1:-1)*s.current)<1e-12,'segment pairing determines winding-current sign');}
 else{assert.equal(readout,'Both on insulation');assert.equal(s.current,0);assert.equal(Math.abs(s.coilCurrent),0);assert.equal(s.torque,0);}
 assert.ok(Math.abs(s.inputPower-s.copperLoss-s.convertedPower)<1e-12,'power identity unchanged');return s;
}
