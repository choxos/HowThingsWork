...
export function createHornModel(){
 ...
 const result=finish(v=>{
  ...
  return {state:{x,velocity,current,elapsed,stage,complete,cycles,breaks,frequency:hz,pressed,contactClosed:contactGap<1e-12,contactGap:contactGap/scale,airGap:.0015-x,peak,trough,leafLength:leafLength(leafSpan),leafSpan,buttonSpringLength,sustained,producedTone:periods.length>=3&&breaks>=3,soundReady:context?.state==='running'&&!audioError},readings:[
   ...
   r('Diaphragm cycles',String(cycles),'Complete periods between upward crossings of the resting center position during a normal powered press. The first crossing sets the reference; subsequent crossings count periods, including startup periods. Decay after release and bypass transients are excluded.'),
   r('Vibration frequency',hz?`${hz.toFixed(0)} Hz (model)`:'Not yet periodic','Reciprocal of the mean of up to eight recent measured cycle periods. The unpowered, undamped mass-spring estimate is about 239 Hz; magnetic feedback and damping change the powered frequency. The final reading retains the measured trial frequency.'),
   r('Time since press',`${(elapsed*1000).toFixed(2)} ms`,'Physical time in this model, including vibration decay after release. One display second advances 10 ms: motion runs at one hundredth speed.'),
   r('Contact breaks',String(breaks),'Closed-to-open transitions while the horn button is held, including startup. They need not equal complete diaphragm cycles. Retracting the contact in setup is not a counted break.'),
   r('Button',pressed?'Pressed':'Released','The external button stays closed for Button hold time. The separate internal contact repeatedly switches during that one press.'),
   r('Interrupter',contactGap<1e-12?'Closed':`Open · ${(contactGap/scale*1000).toFixed(3)} mm gap`,'Actual separation of the contact faces. The bar actuator takes up 0.35 mm of travel before lifting the leaf. The bypass can carry current even while these faces are apart.'),
   r('Coil current',`${current.toFixed(2)} A`,'Illustrative resistance 3 Ω and inductance 3 mH give a 1 ms rise time constant. At 12 V, an uninterrupted path approaches 4 A. Opening uses a prescribed 50 μs decay; arcing is omitted.'),
   r('Center movement',`${(x*1000).toFixed(3)} mm`,'Diaphragm center displacement from rest, shared by the rigidly attached bar. Positive motion approaches the fixed pole; negative motion passes back beyond rest. The clamped outer rim stays fixed.'),
   r('Peak center movement',`${(peak*1000).toFixed(3)} mm`,'Largest displacement toward the fixed pole during this trial, including startup and decay. Retained at completion and cleared by replay, reset or a new setup. This is not peak-to-peak travel or a loudness measurement.'),
   r('Pole air gap',`${((.0015-x)*1000).toFixed(3)} mm`,'Clearance between the actual bar and fixed-pole faces: 1.5 mm at rest minus center movement. Normal vibration keeps a positive gap; this horn does not strike the pole to make sound.'),
   r('Horn sound',!v.sound?'Muted':audioError?'Audio unavailable':sustained?(playing?'Synthesized tone at the modeled frequency':'Paused · sound off'):'Quiet · no established powered oscillation','Opt-in tone uses the measured cycle frequency while playing. Pause, Step and completion are silent. Replay returns to muted. This sawtooth teaching tone does not predict loudness or trumpet resonance.'),
   r('Model limit','Illustrative dynamics; no calibrated loudness','No acoustic resonance solver; transient clicks are omitted.')
  ]};
 });
