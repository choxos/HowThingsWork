...
try{
 await page.goto(`${process.env.SITE_URL||'http://127.0.0.1:5175/'}#machine/electric-horn-moving-iron-bar`);
 await page.getByRole('heading',{name:'Electric-horn moving iron bar',exact:true}).waitFor();
 const isolated=page.getByRole('checkbox',{name:'Isolate selected part',exact:true}),detail=page.locator('.daily-part-detail');
 assert.equal(await isolated.isChecked(),false);assert.match(await detail.textContent(),/Moving iron bar and center fixing/);
 for(const [index,outcome] of [[0,'9 diaphragm cycles · sustained tone produced'],[1,'No sustained tone'],[2,'No sustained tone']]){
  ...
  if(index===0){
   ...
   const label=page.locator('.daily-model-label').filter({hasText:/^Moving iron bar and center fixing$/}),point=()=>label.evaluate(n=>[parseFloat(n.style.left),parseFloat(n.style.top)]),start=await point();
   for(let step=0;step<12;step++){
    await page.getByRole('button',{name:'Advance one step',exact:true}).click();
    const position=await point();assert.ok(Math.hypot(position[0]-start[0],position[1]-start[1])<.01,'selected bar stays framed through its stroke');
   }
 ...
 assert.equal(await isolated.isChecked(),false);assert.match(await detail.textContent(),/Moving iron bar and center fixing/);
 await page.getByRole('button',{name:'The bar’s bolted fixing through the diaphragm center.',exact:true}).click();
 assert.match(await page.locator('.daily-answer').textContent(),/That’s right/);
 await page.screenshot({path:'/tmp/howthingswork-horn-bar-lesson.png',fullPage:true});
 await page.setViewportSize({width:390,height:844});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));
 assert.deepEqual(errors,[]);
 console.log('PASS horn bar browser: dedicated connected route, three stepped experiments and whole-horn outcomes, reset context, quiz and mobile.');
