...
export function createBellModel({springLesson=false,hammerLesson=false,buttonLesson=false}={}){
 ...
 control('holdTime','Button hold time',.2,.8,.05,.35,'s','The mechanism runs at 0.12 times real speed, about one eighth speed. Play presses, holds and releases the button; the time reading shows model time.');
 ...
 let angle=0,velocity=0,current=0,elapsed=0,buttonHeldTime=0,stage='ready',complete=false,strikes=0,contactOpenings=0,lastImpact=-Infinity,armed=true,accumulator=0,lastClock=0;
 ...
 const result=finish(v=>{
  ...
  const age=elapsed-lastImpact;for(let i=0;i<waves.length;i++){const phase=Math.min(1,age/.09+i*.35);waves[i].visible=stage==='running'&&age>=0&&phase<1;waves[i].scale.setScalar(1+phase*.6);waves[i].material.opacity=Math.max(0,.65*(1-phase));}
  const force=Math.min(.6,5e-5*current*current/(.006-lever*Math.sin(angle)+(.04/30)*(1-Math.cos(angle)))**2),closed=pose.gap<1e-9;
  const hammerGap=Math.max(0,Math.hypot(-.4*Math.cos(angle)+1.8*Math.sin(angle)-gongCenter[0],pivotY+.4*Math.sin(angle)+1.8*Math.cos(angle)-gongCenter[1])-.7);
  ...
  return {state:{angle,velocity,current,elapsed,buttonHeldTime,stage,complete,strikes,contactOpenings,pressed,contactClosed:closed,contactGap:pose.gap,force,hammerGap,springTorque:stiffness*angle,springEnergy:.5*stiffness*angle*angle,springRadius,wireLength,buttonWireLength,tongueLength,lastImpact:Number.isFinite(lastImpact)?lastImpact:null,soundReady:audioContext?.state==='running'&&!audioError},readings:[
   r('Your result',resultText),
   ...(springLesson?[r('Spring restoring torque',(stiffness*angle*1000).toFixed(2)+' mN·m','Magnitude of the torque toward the rest stop: κθ with θ in radians and illustrative stiffness κ = 0.054 N·m/rad. It acts even while the magnet pulls inward.'),r('Stored spring energy',(.5*stiffness*angle*angle*1000).toFixed(3)+' mJ','Elastic energy above the unloaded rest position: ½κθ². It rises as the armature loads the spring and falls during return; damping and impacts dissipate mechanical energy.')]:[]),
   ...(hammerLesson?[r('Hammer to rim',(hammerGap*1000/30).toFixed(2)+' mm','Nearest gap between the spherical hammer surface and the circular gong rim in their shared motion plane. Converted from the enlarged model; zero means geometric contact. The display rounds tiny clearances to 0.00 mm, while strikes are counted at the actual impact stop.')]:[]),
   ...(buttonLesson?[r('Button held for',(buttonHeldTime*1000).toFixed(1)+' ms','Accumulated physical time with the door button held down in this trial. It stops increasing after release while Time since press includes the final return. Changing the hold target during playback does not rewrite time already held.')]:[]),
   r('Hammer strikes',String(strikes),'Counted when the actual hammer reaches the gong rim. The displayed motion is slowed; the synthesized tone is a separate illustration of each impact.'),
   r('Contact openings',String(contactOpenings),'Counts each closed-to-open transition caused by armature motion in this trial, including its final return. Retracting the screw in setup is not counted. An opening need not produce a hammer strike.'),
   r('Time since press',(elapsed*1000).toFixed(1)+' ms','Physical time in this model, including the final return after the button releases. Playback runs at 0.12 times real speed, so one display second advances about 120 ms.'),
   r('Button',pressed?'Pressed':'Released','The external button remains down for Button hold time. The separate internal contact can open and close repeatedly during that same press.'),
   r('Moving contact',closed?'Closed':'Open · '+(pose.gap*1000/30).toFixed(2)+' mm gap','Measured separation of the two conducting faces, converted from the enlarged model. The bypass jumper can carry current even while these faces are apart.'),
   r('Coil current',(current*1000).toFixed(0)+' mA','The illustrative coils have 10 Ω resistance and 0.08 H inductance: an 8 ms rise time constant. At 3 V, uninterrupted current approaches 300 mA. Opening uses a prescribed 0.5 ms decay; arcing is not solved.'),
   r('Armature angle',(angle*180/Math.PI).toFixed(1)+'°','Actual rotation from the rest stop. In this model the contact opens near 5.7° and the hammer reaches the gong near 9.2°; inertia carries it onward after the supply breaks.'),
   r('What happens now',complete?'Play again repeats this trial; change a setting for a comparison.':stage==='ready'?'Play presses the button and starts the experiment.':!pressed?'The spring brings the armature back.':v.contact===1?'The open path prevents current.':v.contact===2?'The jumper keeps current on when the contact opens.':closed?'Current builds; the magnet pulls.':'Current fades; inertia and the spring govern the return.'),
   r('Bell sound',!v.sound?'Muted · select Sound on to listen':audioError?'Audio unavailable in this browser':audioContext?.state==='running'?'On · synthesized impact tone':'Waiting for sound permission','Sound is optional and starts only after your choice. Reset and completion replay return to muted; the gong resonance is not calculated.')
  ]};
 });
 const render=result.update;
 result.update=next=>{const before=result.getState().values;render(next);const values=result.getState().values;if(values.sound!==before.sound)setSound(values.sound===1);if(complete&&Object.keys(before).some(k=>before[k]!==values[k])){stage='ready';complete=false;elapsed=0;buttonHeldTime=0;strikes=0;contactOpenings=0;lastImpact=-Infinity;armed=true;}return render();};
 function advance(seconds){if(!Number.isFinite(seconds)||seconds<=0||complete)return render();if(stage==='ready'){stage='running';elapsed=0;buttonHeldTime=0;strikes=0;contactOpenings=0;lastImpact=-Infinity;armed=true;}const v=result.getState().values,previous=strikes,dt=.00005;accumulator+=seconds*.12;
  while(accumulator>=dt-1e-12&&!complete){accumulator-=dt;buttonHeldTime+=Math.max(0,Math.min(dt,v.holdTime-elapsed));elapsed+=dt;const pressed=elapsed<v.holdTime,closed=contactPose(v).gap<1e-9,conducting=pressed&&(v.contact===2||closed);current+=dt*(conducting?(v.voltage-10*current)/.08:-current/.0005);
   ...
   if(closed&&contactPose(v).gap>=1e-9)contactOpenings++;
 ...
 result.reset=()=>{angle=0;velocity=0;current=0;elapsed=0;buttonHeldTime=0;stage='ready';complete=false;strikes=0;contactOpenings=0;lastImpact=-Infinity;armed=true;accumulator=0;lastClock=0;setSound(false);return render(result.defaults);};
 result.advance=advance;result.animate=t=>{if(!Number.isFinite(t))return render();const dt=Math.max(0,t-lastClock);lastClock=t;return advance(dt);};
 result.playback={label:'Ring the bell',stepLabel:'Advance one step',description:'Play presses the door button for the chosen time, then releases it. The mechanism runs at about one eighth speed; the optional bell tone sounds at each impact.',advance,step:()=>advance(.2),complete:()=>complete,blocked:()=>false};
