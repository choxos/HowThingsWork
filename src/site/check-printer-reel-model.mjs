...
const m=mapping.createModel(),part=id=>m.parts.find(p=>p.id===id).object,close=(a,b,e=1e-8)=>assert.ok(Math.abs(a-b)<e,`${a} != ${b}`),TAU=2*Math.PI,perRadian=Math.hypot(C.reelRadius,C.filamentDiameter*C.scale/TAU),initialLength=C.reelTurns*TAU*perRadian/C.scale;
...
const check=()=>{const s=m.getState();assert.deepEqual(s.readings.slice(0,5).map(r=>r.label),['Your result','Filament on reel','Filament consumed','Reel turns unwound','Free supply span']);close(s.reelRemaining+s.filamentConsumed,initialLength);close(s.reelUnwound,s.filamentConsumed*C.scale/(TAU*perRadian));close(s.supplySpan,42);assert.ok(s.reelRemaining>0);close(part('spool').rotation.z,-s.reelUnwound*TAU);const path=part('reel').children.find(x=>x.geometry?.type==='TubeGeometry').geometry.parameters.path;assert.ok(path.getTangent(1).x>0);const start=path.getPoint(0),original=new THREE.Vector3(C.reelRadius*Math.cos(Math.PI/2+C.reelTurns*TAU),C.reelRadius*Math.sin(Math.PI/2+C.reelTurns*TAU),-.31);original.applyAxisAngle(new THREE.Vector3(0,0,1),part('spool').rotation.z);close(start.distanceTo(original),0);return s;};
try{
 ...
 const parent=createPrinterModel();try{
  const compare=()=>{const {readings:a,...sa}=m.getState(),{readings:b,...sb}=parent.getState();assert.deepEqual(sa,sb);assert.deepEqual([...a].sort((x,y)=>x.label.localeCompare(y.label)),[...b].sort((x,y)=>x.label.localeCompare(y.label)));assert.equal(b[1].label,'Nozzle temperature');};
  for(const [i,p] of lesson.tryIt.entries()){prepare(i);parent.reset(p.initialState);parent.update(p.values);compare();for(const dt of [.1,1,8,.03,150]){m.advance(dt);parent.advance(dt);compare();}}
